...
const validateStripeKey = (key) => {
    ...
    if (hasNewline) {
        throw new Error('STRIPE_SECRET_KEY contains newline characters. ' +
            'Fix your Railway/environment variable - do not copy-paste with line breaks.');
    }
    if (hasQuotes) {
        throw new Error('STRIPE_SECRET_KEY is wrapped in quotes. ' +
 ...
 */
const validateWebhookSecretKey = (secret) => {
    if (!secret) {
...
// ═══════════════════════════════════════════════════════════════════════════════
let VALIDATED_STRIPE_KEY = null;
let VALIDATED_WEBHOOK_SECRET = null;
...
if (stripeConfig.valid && VALIDATED_STRIPE_KEY) {
    try {
        stripe = new stripe_1.default(VALIDATED_STRIPE_KEY, {
...
const getStripeOrFail = (res, correlationId) => {
 ...
 */
const isValidStripeCustomerId = (customerId) => {
    if (!customerId)
        return false;
 ...
 */
const getOrCreateStripeCustomer = async (stripeClient, user, correlationId) => {
 ...
 */
const normalizeSubscriptionStatusForCheckout = async (userId) => {
...
const pricingValidation = (0, pricing_1.validatePricingConfiguration)();
...
router.post('/purchase-tokens', auth_1.authenticate, async (req, res) => {
    try {
        const stripeClient = getStripeOrFail(res);
        if (!stripeClient)
            return;
        ...
        const tokenPackagesArray = (0, pricing_1.getTokenPackages)();
        const tokenPackagesMap = {};
...
router.get('/pricing', async (req, res) => {
    try {
        const tiers = (0, pricing_1.getPricingTiers)();
...
router.get('/token-packages', async (req, res) => {
    try {
        const packages = (0, pricing_1.getTokenPackages)();
        res.json({
            success: true,
            data: {
                packages
...
router.post('/create-checkout-session', auth_1.optionalAuth, async (req, res) => {
    ...
    try {
        ...
        if (mode === 'subscription') {
            ...
            const userExists = await database_1.default.user.findUnique({
                where: { id: req.user.id },
        ...
        if (!priceId && tierId) {
            const stripePriceIds = (0, pricing_1.getStripePriceIds)();
 ...
 */
router.post('/create-token-checkout', auth_1.authenticate, async (req, res) => {
    try {
        const stripeClient = getStripeOrFail(res);
        if (!stripeClient)
            return;
...
router.get('/verify-prices', async (req, res) => {
    try {
        ...
        if (!stripeClient)
            return;
        const priceIds = (0, pricing_1.getStripePriceIds)();
        const results = {};
        // Check each price ID
...
router.get('/health', async (req, res) => {
    ...
    const priceValidation = (0, pricing_1.validatePricingConfiguration)();
 ...
 */
router.get('/checkout-session/:sessionId', auth_1.authenticate, async (req, res) => {
    try {
        const stripeClient = getStripeOrFail(res);
        if (!stripeClient)
            return;
...
async function isWebhookEventProcessed(eventId) {
    try {
        const existingEvent = await database_1.default.webhookEvent.findUnique({
            where: { id: eventId }
        });
 ...
 */
async function markWebhookEventProcessed(eventId, eventType, errorMessage) {
    try {
        await database_1.default.webhookEvent.upsert({
            where: { id: eventId },
            create: {
                id: eventId,
                type: eventType,
...
async function cleanupOldWebhookEvents() {
    ...
    try {
        const result = await database_1.default.webhookEvent.deleteMany({
            where: {
...
async function handleSubscriptionUpdate(subscription) {
    ...
    const plan = (0, pricing_1.getPlanFromPriceId)(priceId);
    const subscriptionTier = (0, pricing_1.getSubscriptionTier)(plan);
    const generationsLimit = (0, pricing_1.getGenerationLimits)(plan);
    ...
    try {
        await emailService_1.default.sendSubscriptionConfirmation(user.email, {
            plan: plan,
            billingCycle: billingCycle,
...
async function handleSubscriptionCancellation(subscription) {
    ...
    try {
        await emailService_1.default.sendSubscriptionCancellation(user.email, {
...
async function handleInvoicePaymentSucceeded(invoice) {
    ...
    try {
        await emailService_1.default.sendPaymentReceipt(user.email, {
...
async function handleInvoicePaymentFailed(invoice) {
    ...
    try {
        await emailService_1.default.sendPaymentFailed(user.email, {
            attemptCount,
...
async function handlePaymentActionRequired(invoice) {
    ...
    try {
        await emailService_1.default.sendPaymentActionRequired(user.email, {
            invoiceUrl: invoice.hosted_invoice_url || undefined,
 ...
 */
async function handleTokenPurchaseSuccess(paymentIntent) {
    ...
    const existingPurchase = await database_1.default.tokenPurchase.findUnique({
    ...
    try {
        await emailService_1.default.sendTokenPurchaseConfirmation(user.email, {
            tokenCount,
 ...
 */
router.post('/upgrade-subscription', auth_1.authenticate, async (req, res) => {
    try {
        const stripeClient = getStripeOrFail(res);
        if (!stripeClient)
            return;
 ...
 */
router.get('/preview-upgrade', auth_1.authenticate, async (req, res) => {
    try {
        const stripeClient = getStripeOrFail(res);
        if (!stripeClient)
            return;
        ...
        if (!targetPriceId && newTierId) {
            ...
            const stripePriceIds = (0, pricing_1.getStripePriceIds)();
            const normalizedTier = newTierId.toLowerCase().replace(/-/g, '_').replace(/ /g, '_');
            const normalizedCycle = (billingCycle || user.billingCycle || 'monthly').toLowerCase();
            const tierPriceMap = {
                'starter': normalizedCycle === 'yearly' ? stripePriceIds.STARTER_YEARLY : stripePriceIds.STARTER_MONTHLY,
                'academy': normalizedCycle === 'yearly' ? stripePriceIds.ACADEMY_PLUS_YEARLY : stripePriceIds.ACADEMY_PLUS_MONTHLY,
                'academy_plus': normalizedCycle === 'yearly' ? stripePriceIds.ACADEMY_PLUS_YEARLY : stripePriceIds.ACADEMY_PLUS_MONTHLY,
        ...
        const newPlan = (0, pricing_1.getPlanFromPriceId)(targetPriceId);
        // Calculate proration breakdown
        const prorationItems = upcomingInvoice.lines.data.filter(line => line.proration);
 ...
 */
router.post('/cancel-subscription', auth_1.authenticate, async (req, res) => {
    try {
        const stripeClient = getStripeOrFail(res);
        if (!stripeClient)
            return;
        ...
        if (immediate) {
            ...
            canceledSubscription = await stripeClient.subscriptions.update(user.stripeSubscriptionId, { cancel_at_period_end: true });
        }
 ...
 */
router.post('/reactivate-subscription', auth_1.authenticate, async (req, res) => {
    try {
        const stripeClient = getStripeOrFail(res);
        if (!stripeClient)
            return;
        ...
        const reactivatedSubscription = await stripeClient.subscriptions.update(user.stripeSubscriptionId, { cancel_at_period_end: false });
        // Update local records - use upsert to handle missing AcademySubscription
        await database_1.default.academySubscription.upsert({
            where: { userId: user.id },
            create: {
                userId: user.id,
                tier: user.subscriptionTier || 'free',
 ...
 */
router.get('/subscription-status', auth_1.authenticate, async (req, res) => {
    try {
        const user = await database_1.default.user.findUnique({
            where: { id: req.user.id },
 ...
 */
router.get('/admin/webhook-events', auth_1.authenticate, async (req, res) => {
    try {
        // Verify admin access
        const user = await database_1.default.user.findUnique({
            where: { id: req.user.id }
        ...
        const [events, stats] = await Promise.all([
            ...
            database_1.default.webhookEvent.groupBy({
                by: ['type', 'processed'],
                _count: { id: true },
                where: {
 ...
 */
router.post('/admin/cleanup-webhook-events', auth_1.authenticate, async (req, res) => {
    try {
        const user = await database_1.default.user.findUnique({
            where: { id: req.user.id }
        });
