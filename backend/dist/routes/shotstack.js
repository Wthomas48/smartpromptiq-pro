...
// HELPER FUNCTIONS
// ═══════════════════════════════════════════════════════════════════════════════
async function logVideoCost(userId, action, cost, details) {
    try {
...
router.get('/templates', (_req, res) => {
    ...
    const aspectRatio = _req.query.aspectRatio || null;
...
router.post('/quick-video', async (req, res) => {
    try {
        ...
        const { title, subtitle, templateId, backgroundImage, backgroundColor = '#1a1a2e', textColor = '#ffffff', accentColor = '#6366f1', voiceUrl, musicUrl, musicVolume = 0.3, voiceVolume = 1.0, duration = 15, aspectRatio = '16:9', format = 'mp4', resolution = '1080', textStyle = 'future', } = req.body;
        if (!title) {
...
router.post('/template-video', async (req, res) => {
    try {
        ...
        const { templateId, customizations, voiceUrl, musicUrl, musicVolume = 0.3, } = req.body;
...
router.post('/scenes-video', async (req, res) => {
    try {
        ...
        const { scenes, musicUrl, musicVolume = 0.3, voiceUrl, voiceVolume = 1.0, aspectRatio = '16:9', format = 'mp4', resolution = '1080', transitionType = 'fade', } = req.body;
...
router.post('/intro-outro', async (req, res) => {
    try {
        ...
        // Content
        title, subtitle, channelName, tagline, 
        // Styling
        backgroundImage, backgroundColor = '#1a1a2e', textColor = '#ffffff', accentColor = '#6366f1', textStyle = 'future', logoUrl, 
        // Audio
        musicUrl, musicVolume = 0.6, voiceUrl, voiceVolume = 1.0, 
        // Timing
        duration = 5, fadeIn = 0.5, fadeOut = 1.0, 
        // Output
        aspectRatio = '16:9', format = 'mp4', resolution = '1080', 
        // Effects
        effect = 'zoomIn', transition = 'fade', } = req.body;
        const userId = req.userId;
        const size = getAspectRatioSize(aspectRatio);
 ...
 */
router.post('/intro-outro-pack', async (req, res) => {
    try {
        if (!SHOTSTACK_API_KEY) {
            return res.status(500).json({ message: 'Shotstack API not configured' });
        }
        const { 
        // Content
        title, channelName, introTagline, outroTagline = 'Thanks for watching! Subscribe for more.', 
        // Styling (shared)
        backgroundImage, backgroundColor = '#1a1a2e', textColor = '#ffffff', accentColor = '#6366f1', textStyle = 'future', logoUrl, 
        // Audio
        musicUrl, musicVolume = 0.6, introVoiceUrl, outroVoiceUrl, voiceVolume = 1.0, 
        // Timing
        introDuration = 5, outroDuration = 10, 
        // Output
        aspectRatio = '16:9', format = 'mp4', resolution = '1080', } = req.body;
        const userId = req.userId;
        const size = getAspectRatioSize(aspectRatio);
        ...
        // Submit intro render
        renderPromises.push(fetch(`${SHOTSTACK_BASE_URL}/render`, {
            method: 'POST',
            headers: {
                'x-api-key': SHOTSTACK_API_KEY,
                'Content-Type': 'application/json',
