...
router.post('/generate-prompt', [
    subscriptionAuth_1.authenticateWithSubscription,
    subscriptionAuth_1.trackApiUsage,
    (0, subscriptionAuth_1.requireTokens)(1, 'standard'),
    (0, express_validator_1.body)('category').notEmpty().trim(),
    (0, express_validator_1.body)('answers').isObject(),
    (0, express_validator_1.body)('customization').isObject()
], async (req, res) => {
    try {
...
router.post('/refine-prompt', [
    ...
    subscriptionAuth_1.trackApiUsage,
    (0, subscriptionAuth_1.requireTokens)(1, 'standard'),
    (0, express_validator_1.body)('currentPrompt').notEmpty().trim(),
    (0, express_validator_1.body)('refinementQuery').notEmpty().trim(),
    (0, express_validator_1.body)('category').notEmpty().trim()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
...
function generateRefinementText(refinementQuery, category) {
...
This comprehensive plan provides the structure and flexibility needed for sustainable personal growth. Remember: progress over perfection, consistency over intensity.`
    ...
    const template = categoryTemplates[category] || categoryTemplates['marketing'];
...
router.get('/stats', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const promptsCount = await database_1.default.prompt.count({
        ...
        const categoryStats = await database_1.default.prompt.groupBy({
            by: ['category'],
            where: { userId: req.user.id },
            _count: {
                category: true
        ...
        res.json({
            ...
            data: {
                ...
                aiProvider: aiService_1.default.getProviderStatus(),
                aiConfigured: aiService_1.default.isConfigured()
            }
        });
    }
    catch (error) {
...
router.post('/test-email', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // Send test email
        const result = await emailService_1.default.sendTestEmail(user.email);
        res.json({
            success: true,
            message: 'Test email sent successfully',
            emailSent: result,
            emailStatus: emailService_1.default.getStatus()
        });
    }
    catch (error) {
...
router.get('/system-status', auth_1.authenticate, async (req, res) => {
    try {
        const user = await database_1.default.user.findUnique({
            where: { id: req.user.id }
        });
        const systemStatus = {
            ...
            services: {
                ...
                email: {
                    ...emailService_1.default.getStatus(),
                    status: 'operational'
                },
                database: {
                    status: 'operational',
