...
router.get('/subscription', auth_1.authenticate, async (req, res) => {
    try {
        const academySubscription = await database_1.default.academySubscription.findUnique({
...
router.post('/cancel', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const subscription = await stripe.subscriptions.update(academySubscription.stripeSubscriptionId, {
            cancel_at_period_end: true
        });
        // Update local record
        await database_1.default.academySubscription.update({
            where: { userId: req.user.id },
            data: {
 ...
 */
router.post('/reactivate', auth_1.authenticate, async (req, res) => {
    try {
        const academySubscription = await database_1.default.academySubscription.findUnique({
            where: { userId: req.user.id }
        });
        ...
        await stripe.subscriptions.update(academySubscription.stripeSubscriptionId, {
 ...
 */
router.get('/portal', auth_1.authenticate, async (req, res) => {
    try {
        const user = await database_1.default.user.findUnique({
            where: { id: req.user.id }
        });
 ...
 */
router.get('/verify-session/:sessionId', auth_1.authenticate, async (req, res) => {
    try {
