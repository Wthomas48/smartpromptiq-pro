...
router.post('/mvp-planning', [
    (0, express_validator_1.body)('answers').optional().isObject(),
...
router.post('/:category/:template', [
    ...
    (0, subscriptionAuth_1.requireTokens)(1, 'category-specific'),
    (0, express_validator_1.body)('answers').optional().isObject(),
    (0, express_validator_1.body)('preferences').optional().isObject()
], async (req, res) => {
    try {
        ...
        const prompt = await aiService_1.default.generateCategoryPrompt({
            category,
            template,
            answers,
            preferences,
        ...
        // Save to database
        const savedPrompt = await database_1.default.prompt.create({
            data: {
...
function generateMVPPlanningPrompt(answers, preferences) {
...
function generateUXDesignPrompt(answers, preferences) {
...
function generateCompetitiveAnalysisPrompt(answers, preferences) {
