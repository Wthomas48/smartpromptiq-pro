...
router.get('/pricing', async (req, res) => {
    try {
        res.json({
            success: true,
            data: {
                tokenPackages: costs_1.TOKEN_PACKAGES,
                subscriptionTokens: costs_1.SUBSCRIPTION_TOKENS,
                tokenCosts: costs_1.TOKEN_COSTS,
            },
        });
    }
    catch (error) {
...
router.get('/admin/dashboard', async (req, res) => {
    ...
    try {
        const systemStats = await (0, costTracking_1.getSystemCostStats)();
        if (!systemStats) {
        ...
        res.json({
            ...
            data: {
                ...
                costByProvider,
                costByFeature,
                controlFlags: costs_1.COST_CONTROL_FLAGS,
                apiCosts: costs_1.API_COSTS,
            },
        });
    }
    catch (error) {
 ...
 */
router.post('/admin/set-flags', async (req, res) => {
    const isAdmin = req.user?.isAdmin;
    if (!isAdmin) {
        return res.status(403).json({ error: 'Admin access required' });
    }
 ...
 */
router.get('/admin/high-spenders', async (req, res) => {
    const isAdmin = req.user?.isAdmin;
    if (!isAdmin) {
        return res.status(403).json({ error: 'Admin access required' });
    }
    ...
    try {
        ...
        const enrichedData = highSpenders.map(s => ({
            ...
            isAboveThreshold: (s._sum.cost || 0) > costs_1.COST_ALERTS.perUser.daily,
        }));
        res.json({
            success: true,
            data: {
                period,
                startDate,
                threshold: costs_1.COST_ALERTS.perUser,
                users: enrichedData,
            },
        });
    }
 ...
 */
router.post('/admin/emergency-shutdown', async (req, res) => {
    const isAdmin = req.user?.isAdmin;
    if (!isAdmin) {
        return res.status(403).json({ error: 'Admin access required' });
    }
    ...
    try {
        ...
        costs_1.COST_CONTROL_FLAGS.enableGPT4 = false;
        costs_1.COST_CONTROL_FLAGS.enableDALLE3 = false;
        costs_1.COST_CONTROL_FLAGS.enableElevenLabs = false;
        costs_1.COST_CONTROL_FLAGS.enableSuno = false;
...
router.post('/admin/restore', async (req, res) => {
    ...
    try {
        // Re-enable features
        costs_1.COST_CONTROL_FLAGS.enableGPT4 = true;
        costs_1.COST_CONTROL_FLAGS.enableDALLE3 = true;
        costs_1.COST_CONTROL_FLAGS.enableElevenLabs = true;
        costs_1.COST_CONTROL_FLAGS.enableSuno = true;
 ...
 */
router.post('/admin/test-alert', async (req, res) => {
    const isAdmin = req.user?.isAdmin;
    if (!isAdmin) {
        return res.status(403).json({ error: 'Admin access required' });
    }
    ...
    try {
        const result = await (0, costAlertService_1.sendCostAlert)({
            level: level,
            period: period,
            currentCost: 150,
            threshold: 200,
...
router.post('/admin/check-alerts', async (req, res) => {
    ...
    try {
        await (0, costAlertService_1.checkAndSendCostAlerts)();
        res.json({
            success: true,
