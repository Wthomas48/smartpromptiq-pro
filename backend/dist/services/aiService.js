...
Return the complete refined prompt that incorporates the user's requested improvements.`;
    ...
    generateMockRefinement(currentPrompt, refinementQuery, category, originalAnswers) {
        ...
        if (refinementQuery.toLowerCase().includes('specific') || refinementQuery.toLowerCase().includes('target audience')) {
            refinedPrompt = currentPrompt.replace(/your target audience|your ideal customers/g, `${originalAnswers?.targetAudience || 'your specific target demographic'}`);
    ...
    async generateSuggestions(category) {
        ...
        if (category) {
            return suggestions.filter(s => s.title.toLowerCase().includes(category.toLowerCase()) ||
