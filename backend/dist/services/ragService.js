...
async function retrieveRelevantChunks(documentId, queryEmbedding, topK = 5) {
    const chunks = await database_1.prisma.documentChunk.findMany({
        where: { documentId },
        select: {
            id: true,
            content: true,
...
function buildRAGPrompt(question, chunks, chatHistory = []) {
...
async function generateRAGResponse(prompt, preferredProvider) {
...
async function chat(documentId, userId, message, options = {}) {
    ...
    let chatHistory = [];
    if (existingChatId) {
        const prevMessages = await database_1.prisma.documentChatMessage.findMany({
