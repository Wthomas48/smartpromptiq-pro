...
function getRoomName(documentType, documentId) {
    return `doc:${documentType}:${documentId}`;
}
function assignColor(roomName) {
...
function registerCollaborationHandlers(io, socket) {
    ...
    socket.on('join-document', async ({ documentType, documentId, teamId }) => {
        if (!documentType || !documentId || !teamId)
            return;
    ...
    socket.on('document-update', async ({ documentType, documentId, changes, version }) => {
        if (!documentType || !documentId || !changes)
            return;
        const roomName = getRoomName(documentType, documentId);
        ...
        if (documentType === 'prompt') {
            try {
                ...
                const prompt = await database_1.default.prompt.findUnique({
                    where: { id: documentId },
                });
                if (!prompt) {
                ...
                if (changes.field === 'title')
                    updateData.title = changes.value;
                else if (changes.field === 'content')
                    updateData.content = changes.value;
                else if (changes.field === 'category')
                    updateData.category = changes.value;
    ...
    socket.on('cursor-move', ({ documentType, documentId, position }) => {
        if (!documentType || !documentId || !position)
            return;
        const roomName = getRoomName(documentType, documentId);
        const editors = documentEditors.get(roomName);
    ...
    socket.on('selection-change', ({ documentType, documentId, selection }) => {
        if (!documentType || !documentId || !selection)
            return;
        const roomName = getRoomName(documentType, documentId);
        const editors = documentEditors.get(roomName);
        const editor = editors?.get(user.id);
...
function leaveDocument(io, socket, documentType, documentId, user) {
    const roomName = getRoomName(documentType, documentId);
