...
console.log('🔍 Absolute path:', path_1.default.resolve(clientDistPath));
// Check if the directory exists at startup
if (fs_1.default.existsSync(clientDistPath)) {
    console.log('✅ Frontend build directory exists');
    const files = fs_1.default.readdirSync(clientDistPath);
...
app.get('/api/suggestions/personalized', async (req, res) => {
    try {
        ...
        const aiService = require('./services/aiService').default;
        const suggestions = await aiService.generateSuggestions(category);
        res.json({
            suggestions,
            category,
            success: true,
