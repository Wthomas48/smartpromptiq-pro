...
exports.costMiddleware = void 0;
exports.trackCost = trackCost;
exports.preCheckTokens = preCheckTokens;
...
function trackCost(options) {
    return async (req, res, next) => {
        ...
        try {
            ...
            const tier = (0, costs_1.getSubscriptionTier)(user.subscriptionTier || 'free');
            const tokenCost = (0, costs_1.getTokenCost)(options.category, options.feature);
            ...
            const dailyLimit = (0, costs_1.checkUsageLimit)(tier, options.feature, cachedUsage.daily, 'daily');
            ...
            if (costs_1.COST_CONTROL_FLAGS.enableCostThrottling) {
                resetDailyCostIfNeeded();
                if (dailyCostTotal >= costs_1.COST_ALERTS.daily.shutdown) {
 ...
 */
async function processCostAfterResponse(req, responseBody) {
    const tracking = req.costTracking;
    if (!tracking)
        return;
    ...
    try {
        ...
        if (costs_1.COST_CONTROL_FLAGS.logAllAPICosts) {
            console.log(`💰 Cost: ${(0, costs_1.formatCost)(actualCost)} | Tokens: ${tokenCost} | User: ${userId} | Feature: ${options.feature}`);
        }
    }
    catch (error) {
 ...
 */
async function preCheckTokens(req, res, next, category, feature) {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: 'Authentication required' });
    }
    try {
        ...
        const tokenCost = (0, costs_1.getTokenCost)(category, feature);
...
async function getUserUsageStats(userId) {
    ...
    try {
        ...
        const tier = (0, costs_1.getSubscriptionTier)(user?.subscriptionTier || 'free');
        const limits = costs_1.USAGE_LIMITS[tier];
        return {
            daily: {
 ...
 */
async function getSystemCostStats() {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const monthStart = new Date(today.getFullYear(), today.getMonth(), 1);
    try {
        ...
        return {
            daily: {
                ...
                alerts: {
                    warning: (dailyCosts._sum.cost || 0) >= costs_1.COST_ALERTS.daily.warning,
                    critical: (dailyCosts._sum.cost || 0) >= costs_1.COST_ALERTS.daily.critical,
                    shutdown: (dailyCosts._sum.cost || 0) >= costs_1.COST_ALERTS.daily.shutdown,
                },
            },
            monthly: {
                ...
                alerts: {
                    warning: (monthlyCosts._sum.cost || 0) >= costs_1.COST_ALERTS.monthly.warning,
                    critical: (monthlyCosts._sum.cost || 0) >= costs_1.COST_ALERTS.monthly.critical,
                    shutdown: (monthlyCosts._sum.cost || 0) >= costs_1.COST_ALERTS.monthly.shutdown,
                },
            },
            topUsers,
            thresholds: costs_1.COST_ALERTS,
        };
    }
    catch (error) {
...
// HELPER FUNCTIONS
// ═══════════════════════════════════════════════════════════════════════════════
function checkCostControlFlags(options) {
    const { provider, model } = options;
    // Check if expensive providers are enabled
    if (provider === 'openai' && model?.includes('gpt-4') && !costs_1.COST_CONTROL_FLAGS.enableGPT4) {
        return false;
    }
    if (provider === 'openai' && model?.includes('dall-e-3') && !costs_1.COST_CONTROL_FLAGS.enableDALLE3) {
        return false;
    }
    if (provider === 'elevenlabs' && !costs_1.COST_CONTROL_FLAGS.enableElevenLabs) {
        return false;
    }
    if (provider === 'suno' && !costs_1.COST_CONTROL_FLAGS.enableSuno) {
        return false;
    }
    return true;
}
...
async function checkCostAlerts(cost, userId) {
    if (!costs_1.COST_CONTROL_FLAGS.alertOnHighCost)
        return;
    // Check daily threshold
    if (dailyCostTotal >= costs_1.COST_ALERTS.daily.warning && dailyCostTotal < costs_1.COST_ALERTS.daily.critical) {
        console.warn(`⚠️ WARNING: Daily cost threshold reached: ${(0, costs_1.formatCost)(dailyCostTotal)}`);
    }
    else if (dailyCostTotal >= costs_1.COST_ALERTS.daily.critical) {
        console.error(`🚨 CRITICAL: Daily cost critical threshold: ${(0, costs_1.formatCost)(dailyCostTotal)}`);
    }
    // Trigger email alert check (async, non-blocking)
    (0, costAlertService_1.checkAndSendCostAlerts)().catch(err => {
    ...
    if (userId && cost > 0) {
        ...
        if (totalUserCost >= costs_1.COST_ALERTS.perUser.daily) {
            ...
            if (user) {
                (0, costAlertService_1.sendUserHighUsageAlert)(userId, user.email, totalUserCost).catch(err => {
 ...
 */
exports.costMiddleware = {
    // Prompt generation
    prompt: (feature = 'standard') => trackCost({
        category: 'prompt',
        feature,
        provider: 'openai',
    ...
    // Voice generation
    voice: (provider = 'elevenlabs') => trackCost({
        category: 'voice',
        feature: `${provider}-standard`,
        provider,
    }),
    // Music generation
    music: (feature = 'instrumental') => trackCost({
        category: 'music',
        feature,
        provider: 'suno',
    }),
    // Image generation
    image: (feature = 'sdxl') => trackCost({
        category: 'image',
        feature,
        provider: feature.includes('dalle') ? 'openai' : 'stability',
    }),
    // BuilderIQ
    builderiq: (feature = 'blueprint-standard') => trackCost({
        category: 'builderiq',
        feature,
        provider: 'openai',
        model: 'gpt-4o',
    ...
    // Academy
    academy: (feature = 'playground-run') => trackCost({
        category: 'academy',
        feature,
...
exports.default = {
    ...
    getUserUsageStats,
    getSystemCostStats,
    costMiddleware: exports.costMiddleware,
};
