 ...
 */
const validateRole = (role: string | undefined | null): ValidRole => {
  if (!role) return 'USER';
 ...
 */
const verifySupabaseToken = (token: string): { sub: string; email: string; role?: string } | null => {
  try {
 ...
 */
const findOrCreateSupabaseUser = async (supabaseUser: { sub: string; email: string; role?: string }) => {
  // Validate inputs
