...
router.post('/', [
  ...
  body('slug').notEmpty().trim().isLength({ min: 2, max: 50 })
    .matches(/^[a-z0-9-]+$/).withMessage('Slug must be lowercase alphanumeric with hyphens'),
  body('description').optional().trim().isLength({ max: 500 }),
  body('systemPrompt').notEmpty().trim().isLength({ min: 10, max: 10000 }),
  body('provider').optional().isIn(['anthropic', 'openai']),
  body('model').optional().isString(),
  body('temperature').optional().isFloat({ min: 0, max: 2 }),
  body('maxTokens').optional().isInt({ min: 100, max: 8000 }),
  body('welcomeMessage').optional().trim().isLength({ max: 500 }),
  body('suggestedQueries').optional().isArray()
...
router.patch('/:id', [
  ...
  body('description').optional().trim().isLength({ max: 500 }),
  body('systemPrompt').optional().trim().isLength({ min: 10, max: 10000 }),
  body('provider').optional().isIn(['anthropic', 'openai']),
  body('model').optional().isString(),
  body('temperature').optional().isFloat({ min: 0, max: 2 }),
  body('maxTokens').optional().isInt({ min: 100, max: 8000 }),
...
router.post('/:id/api-keys', [
  ...
  body('rateLimitPerMinute').optional().isInt({ min: 1, max: 1000 }),
 ...
 * Revoke an API key
 */
router.delete('/:id/api-keys/:keyId', authenticate, async (req: AuthRequest, res: any) => {
  try {
 ...
 */
router.get('/:id/conversations', authenticate, async (req: AuthRequest, res: any) => {
  try {
    const agent = await prisma.agent.findFirst({
      where: {
        id: req.params.id,
 ...
 */
router.get('/:id/analytics', authenticate, async (req: AuthRequest, res: any) => {
  try {
    const agent = await prisma.agent.findFirst({
      where: {
        id: req.params.id,
 ...
 */
router.get('/:id/embed-code', authenticate, async (req: AuthRequest, res: any) => {
  try {
    const agent = await prisma.agent.findFirst({
      where: {
        id: req.params.id,
