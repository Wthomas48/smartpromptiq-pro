...
async function logElevenLabsCost(userId: string | null, action: string, cost: number, tokensUsed: number, model: string) {
  try {
...
const ELEVENLABS_VOICES: Record<string, { id: string; name: string; gender: string; style: string; description: string }> = {
  // ==================== FEMALE VOICES (11) ====================
...
const VOICE_CATEGORIES: Record<string, string[]> = {
...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    const {
      ...
      settings,
      category,
    } = req.body as ElevenLabsGenerateRequest;
    ...
    // Get voice ID
    const voice = ELEVENLABS_VOICES[voiceName as keyof typeof ELEVENLABS_VOICES];
    ...
    const actualAPICost = (text.length / 1000) * API_COSTS.elevenlabs['standard'];
 ...
 */
router.post('/academy/generate', async (req: Request, res: Response) => {
  try {
    const {
      lessonContent,
      lessonTitle,
    ...
    const academyVoice = ELEVENLABS_VOICES[voiceName as keyof typeof ELEVENLABS_VOICES]
 ...
 */
router.post('/page/narrate', async (req: Request, res: Response) => {
  try {
    const {
      content,
      pageTitle,
    ...
    const pageVoiceMap: Record<string, string> = {
      academy: 'rachel',
      dashboard: 'emily',
      pricing: 'domi',
    ...
    const voice = ELEVENLABS_VOICES[selectedVoice as keyof typeof ELEVENLABS_VOICES];
 ...
 */
router.get('/voices/:category', (req: Request, res: Response) => {
  const { category } = req.params;
  const voiceKeys = VOICE_CATEGORIES[category as keyof typeof VOICE_CATEGORIES] || VOICE_CATEGORIES.academy;

  const voices = voiceKeys.map(key => ({
    key,
    ...ELEVENLABS_VOICES[key as keyof typeof ELEVENLABS_VOICES],
  }));
...
router.post('/batch-generate', async (req: Request, res: Response) => {
  try {
    ...
    const voice = ELEVENLABS_VOICES[voiceName as keyof typeof ELEVENLABS_VOICES] || ELEVENLABS_VOICES.rachel;

    console.log(`🎙️ Batch Generation: ${items.length} items with ${voiceName}`);

    const results = await Promise.all(
      items.map(async (item: { text: string; id?: string }, index: number) => {
        try {
          const response = await fetch(`${ELEVENLABS_BASE_URL}/text-to-speech/${voice.id}`, {
            method: 'POST',
            headers: {
...
const SOUND_EFFECT_PRESETS: Record<string, { prompt: string; duration: number; description: string }> = {
  // Transitions
 ...
 */
router.post('/sound-effects/generate', async (req: Request, res: Response) => {
  try {
    const {
      prompt,
      duration = 2.0,
      promptInfluence = 0.3,
    } = req.body as SoundEffectRequest;
    ...
    const sfxCost = API_COSTS.elevenlabs['sound-effect'];
 ...
 */
router.post('/sound-effects/preset', async (req: Request, res: Response) => {
  try {
    const { presetId, customDuration } = req.body;

    const preset = SOUND_EFFECT_PRESETS[presetId as keyof typeof SOUND_EFFECT_PRESETS];
    if (!preset) {
      return res.status(400).json({
 ...
 */
router.get('/sound-effects/presets', (_req: Request, res: Response) => {
 ...
 */
router.get('/sound-effects/categories', (_req: Request, res: Response) => {
  res.json({
    categories: SOUND_EFFECT_CATEGORIES,
    tips: {
 ...
 */
router.post('/sound-effects/batch', async (req: Request, res: Response) => {
  try {
    ...
    const results = await Promise.all(
      effects.map(async (effect: { prompt: string; duration?: number; id?: string }, index: number) => {
        try {
          const response = await fetch(`${ELEVENLABS_BASE_URL}/sound-generation`, {
            method: 'POST',
            headers: {
 ...
 */
router.post('/music/voice-over', async (req: Request, res: Response) => {
  try {
    const {
      script,
      voiceName = 'rachel',
    ...
    const pacingSettings: Record<string, { stability: number; similarity: number }> = {
 ...
 */
router.get('/music/recommendations', (_req: Request, res: Response) => {
