...
router.get('/personalized', authenticate, [
  query('category').optional().trim(),
  query('limit').optional().isInt({ min: 1, max: 50 }).withMessage('Limit must be between 1 and 50')
], async (req, res) => {
  try {
...
router.post('/interaction', authenticate, [
  body('suggestionId').notEmpty().trim().withMessage('Suggestion ID is required'),
  body('action').isIn(['select', 'copy', 'dismiss', 'favorite']).withMessage('Invalid action')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
router.get('/category/:category', authenticate, [
  query('limit').optional().isInt({ min: 1, max: 50 }).withMessage('Limit must be between 1 and 50')
], async (req, res) => {
  try {
    const errors = validationResult(req);
...
router.get('/analytics', authenticate, async (req, res) => {
  try {
    ...
    const analytics = {
      ...
      mostPopularCategories: Object.entries(
        mockSuggestions.reduce((acc: Record<string, number>, s) => {
      ...
      averageTokens: Math.round(
        mockSuggestions.reduce((acc, s) => acc + s.estimatedTokens, 0) / mockSuggestions.length
      )
    };
