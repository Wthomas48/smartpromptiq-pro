...
export async function sendDiscordWebhook(content: WebhookContent): Promise<boolean> {
...
export async function notifyNewUser(user: { username?: string; email: string }) {
...
export async function notifyPlanUpgrade(user: { username?: string; email: string }, plan: string) {
  await sendDiscordWebhook({
    title: 'Plan Upgrade!',
...
export async function notifySupportRequest(data: { subject: string; name: string; category: string; email: string }) {
  await sendDiscordWebhook({
