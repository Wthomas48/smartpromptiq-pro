...
async function logMusicCost(userId: string | null, action: string, cost: number, tokensUsed: number, model: string) {
  try {
...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    const {
      ...
      style,
      purpose = 'full_track',
    } = req.body as MusicGenerationRequest;
    ...
    const actualAPICost = withVocals
      ? API_COSTS.suno['song-full']
      : purpose === 'intro_jingle'
      ? API_COSTS.suno['jingle']
      : API_COSTS.suno['song-instrumental'];
...
router.get('/status/:trackId', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.get(`${AI_MUSIC_API_URL}/suno/task/${trackId}`, {
      headers: {
        'Authorization': `Bearer ${AI_MUSIC_API_KEY}`,
      },
    });
...
router.post('/stems', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.post(
      `${AI_MUSIC_API_URL}/stem/separate`,
      { audio_url: audioUrl },
      {
        headers: {
          'Authorization': `Bearer ${AI_MUSIC_API_KEY}`,
...
function buildMusicPrompt({
  ...
  userPrompt: string;
  genre: string;
  genrePreset: typeof GENRE_PRESETS[keyof typeof GENRE_PRESETS];
  mood?: string;
  style?: string;
  duration: number;
  purpose: string;
  ...
  prompt += `. Purpose: ${purposeContext[purpose as keyof typeof purposeContext] || purposeContext.full_track}`;
...
function getDemoTrackUrl(genre: string, _purpose: string): string {
...
router.post('/mix', async (req: Request, res: Response) => {
  try {
    ...
    const voiceTracks = tracks.filter((t: any) => t.type === 'voice');
    const musicTracks = tracks.filter((t: any) => t.type === 'music');

    const totalDuration = Math.max(...tracks.map((t: any) => (t.startTime || 0) + t.duration));
 ...
 */
router.post('/voice-overlay', async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/tracks/:id/share', async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: 'Authentication required' });
