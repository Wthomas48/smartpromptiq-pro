...
interface CostAlertData {
  level: 'warning' | 'critical' | 'shutdown';
  period: 'daily' | 'monthly';
  currentCost: number;
  threshold: number;
  percentUsed: number;
  topSpenders?: Array<{ userId: string; email: string; cost: number }>;
  timestamp: Date;
}
 ...
 */
function buildAlertEmailHtml(data: CostAlertData): string {
  const levelColors = {
    warning: '#f59e0b',
    critical: '#ef4444',
    shutdown: '#7f1d1d',
 ...
 */
export async function sendCostAlert(data: CostAlertData): Promise<boolean> {
 ...
 */
export async function checkAndSendCostAlerts(): Promise<void> {
  if (!COST_CONTROL_FLAGS.alertOnHighCost) {
    return;
  }
  ...
  try {
    ...
    if (dailyCost >= COST_ALERTS.daily.shutdown) {
      await sendCostAlert({
        level: 'shutdown',
        period: 'daily',
        currentCost: dailyCost,
        threshold: COST_ALERTS.daily.shutdown,
        percentUsed: (dailyCost / COST_ALERTS.daily.shutdown) * 100,
        topSpenders: topSpendersList,
        timestamp: new Date(),
      });
    } else if (dailyCost >= COST_ALERTS.daily.critical) {
      await sendCostAlert({
        level: 'critical',
        period: 'daily',
        currentCost: dailyCost,
        threshold: COST_ALERTS.daily.critical,
        percentUsed: (dailyCost / COST_ALERTS.daily.critical) * 100,
        topSpenders: topSpendersList,
        timestamp: new Date(),
      });
    } else if (dailyCost >= COST_ALERTS.daily.warning) {
      await sendCostAlert({
        level: 'warning',
        period: 'daily',
        currentCost: dailyCost,
        threshold: COST_ALERTS.daily.warning,
        percentUsed: (dailyCost / COST_ALERTS.daily.warning) * 100,
        topSpenders: topSpendersList,
        timestamp: new Date(),
      });
    }
    ...
    if (monthlyCost >= COST_ALERTS.monthly.shutdown) {
      await sendCostAlert({
        level: 'shutdown',
        period: 'monthly',
        currentCost: monthlyCost,
        threshold: COST_ALERTS.monthly.shutdown,
        percentUsed: (monthlyCost / COST_ALERTS.monthly.shutdown) * 100,
        timestamp: new Date(),
      });
    } else if (monthlyCost >= COST_ALERTS.monthly.critical) {
      await sendCostAlert({
        level: 'critical',
        period: 'monthly',
        currentCost: monthlyCost,
        threshold: COST_ALERTS.monthly.critical,
        percentUsed: (monthlyCost / COST_ALERTS.monthly.critical) * 100,
        timestamp: new Date(),
      });
    } else if (monthlyCost >= COST_ALERTS.monthly.warning) {
      await sendCostAlert({
        level: 'warning',
        period: 'monthly',
        currentCost: monthlyCost,
        threshold: COST_ALERTS.monthly.warning,
        percentUsed: (monthlyCost / COST_ALERTS.monthly.warning) * 100,
        timestamp: new Date(),
      });
    }
  } catch (error) {
 ...
 */
export async function sendUserHighUsageAlert(
  userId: string,
  userEmail: string,
  dailyCost: number
): Promise<boolean> {
  ...
  if (dailyCost < COST_ALERTS.perUser.daily) {
    return false;
  }
    ...
    const html = `
      ...
      <body style="font-family: sans-serif; padding: 20px;">
        ...
        <div style="background: #fef3c7; padding: 15px; border-radius: 8px; margin: 15px 0;">
          ...
          <p style="margin: 5px 0 0 0;"><strong>Threshold:</strong> $${COST_ALERTS.perUser.daily.toFixed(2)}</p>
        </div>
